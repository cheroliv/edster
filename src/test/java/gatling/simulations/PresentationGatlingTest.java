 ...
 */
public class PresentationGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all presentations").get("/api/presentations").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new presentation")
                        ...
                        .body(
                            StringBody(
                                ...
                                "\"plan\": \"SAMPLE_TEXT\"" +
                                ", \"uri\": \"SAMPLE_TEXT\"" +
                                ", \"promptUserMessage\": \"SAMPLE_TEXT\"" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_presentation_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created presentation").get("${new_presentation_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created presentation").delete("${new_presentation_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Presentation entity").exec(scn);
