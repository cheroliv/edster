 ...
 */
public class WorkspaceGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all workspaces").get("/api/workspaces").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new workspace")
                        .post("/api/workspaces")
                        ...
                        .body(
                            StringBody(
                                ...
                                ", \"promptSystemMessage\": \"SAMPLE_TEXT\"" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_workspace_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created workspace").get("${new_workspace_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created workspace").delete("${new_workspace_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Workspace entity").exec(scn);
