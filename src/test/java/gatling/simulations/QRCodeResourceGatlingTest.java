...
public class QRCodeResourceGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all qRCodeResources").get("/api/qr-code-resources").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new qRCodeResource")
                        .post("/api/qr-code-resources")
                        .headers(headersHttpAuthenticated)
                        .body(StringBody("{" + "\"type\": \"IMAGE\"" + ", \"uri\": \"SAMPLE_TEXT\"" + ", \"data\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_qRCodeResource_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created qRCodeResource").get("${new_qRCodeResource_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created qRCodeResource").delete("${new_qRCodeResource_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the QRCodeResource entity").exec(scn);
