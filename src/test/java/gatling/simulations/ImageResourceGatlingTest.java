 ...
 */
public class ImageResourceGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all imageResources").get("/api/image-resources").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new imageResource")
                        .post("/api/image-resources")
                        .headers(headersHttpAuthenticated)
                        .body(
                            StringBody(
                                "{" + "\"type\": \"IMAGE\"" + ", \"uri\": \"SAMPLE_TEXT\"" + ", \"resolution\": \"SAMPLE_TEXT\"" + "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created imageResource").get("${new_imageResource_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created imageResource").delete("${new_imageResource_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the ImageResource entity").exec(scn);
