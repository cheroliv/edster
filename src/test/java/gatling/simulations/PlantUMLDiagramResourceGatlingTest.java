...
/**
 * Performance test for the PlantUMLDiagramResource entity.
 *
 * @see <a href="https://github.com/jhipster/generator-jhipster/tree/v8.9.0/generators/gatling#logging-tips">Logging tips</a>
 */
public class PlantUMLDiagramResourceGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all plantUMLDiagramResources")
                    .get("/api/plant-uml-diagram-resources")
                    .headers(headersHttpAuthenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new plantUMLDiagramResource")
                        .post("/api/plant-uml-diagram-resources")
                        .headers(headersHttpAuthenticated)
                        .body(
                            StringBody("{" + "\"type\": \"IMAGE\"" + ", \"uri\": \"SAMPLE_TEXT\"" + ", \"umlCode\": \"SAMPLE_TEXT\"" + "}")
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_plantUMLDiagramResource_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created plantUMLDiagramResource")
                            .get("${new_plantUMLDiagramResource_url}")
                            .headers(headersHttpAuthenticated)
                    ).pause(10)
                )
                .exec(
                    http("Delete created plantUMLDiagramResource")
                        .delete("${new_plantUMLDiagramResource_url}")
                        .headers(headersHttpAuthenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the PlantUMLDiagramResource entity").exec(scn);
