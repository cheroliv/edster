...
public class LinkResourceGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all linkResources").get("/api/link-resources").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new linkResource")
                        .post("/api/link-resources")
                        .headers(headersHttpAuthenticated)
                        .body(
                            StringBody("{" + "\"type\": \"IMAGE\"" + ", \"uri\": \"SAMPLE_TEXT\"" + ", \"target\": \"SAMPLE_TEXT\"" + "}")
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_linkResource_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created linkResource").get("${new_linkResource_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created linkResource").delete("${new_linkResource_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the LinkResource entity").exec(scn);
