...
public interface PresentationRepository extends ReactiveCrudRepository<Presentation, Long>, PresentationRepositoryInternal {
    ...
    @Query("SELECT * FROM presentation entity WHERE entity.workspace_id = :id")
    Flux<Presentation> findByWorkspace(Long id);

    @Query("SELECT * FROM presentation entity WHERE entity.workspace_id IS NULL")
    Flux<Presentation> findAllWhereWorkspaceIsNull();
